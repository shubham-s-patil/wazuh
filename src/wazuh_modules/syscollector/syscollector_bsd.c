    ...
    if ((fp = fopen(filepath, "r"))) {
                ...
                while(fgets(read_buff, OS_MAXSTR - 1, fp) != NULL) {
                    ...
                    if (strstr(read_buff, "CFBundleName")) {
                        ...
                        if (strstr(read_buff, "<string>")){
                            ...
                            if (check_vendor_package_mac(_parts[0], &vendor_name, &package_name)) {
        ...
        if (invalid) {
            ...
            *end = '\0';
            ...
            if (check_vendor_package_mac(program_name, &vendor_name, &package_name)) {
                cJSON_AddStringToObject(package, "name", package_name);
                os_free(package_name);
                if (vendor_name) {
                    cJSON_AddStringToObject(package, "vendor", vendor_name);
...
/* This function separate vendor's name from macOS package's name, if it includes the vendor. */
int check_vendor_package_mac(const char * source_package, char ** vendor_name, char ** package_name) {
    ...
    if (next = wstr_chr(package_cpy, ' '), !next) {
        ...
        *next++ = '\0';
        if (!strcmp(package_cpy, "Microsoft") || !strcmp(package_cpy, "VMware")) {
            os_strdup(package_cpy, *vendor_name);
            os_strdup(next, *package_name);
        } else if (!strcmp(package_cpy, "1Password")) {
